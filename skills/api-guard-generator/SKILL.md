...
description: Analyze an API and design an entropy guard for it. Assesses the contract triangle (spec vs implementation vs client expectations), error consistency, and versioning entropy. Use directly when you know the target is an API, or via the entropy-assessment entry point.
metadata:
  version: "0.1.0"
---
...
Analyze an API and design an entropy guard appropriate for it. This is a domain-specific variant of the [entropy assessment](../entropy-assessment/SKILL.md) skill, tuned for the ways API contracts uniquely decay.
